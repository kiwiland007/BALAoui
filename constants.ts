...
import type { Product, User, Review, Transaction, Order } from './types';
import { ProductCategory, ProductCondition, ProductStatus, TransactionType, TransactionStatus, OrderStatus } from './types';
...
export const mockProducts: Product[] = [
  ...
  { id: 'p16', title: 'Polo Lacoste', description: 'Polo classique, couleur bleu marine.', price: 280, category: ProductCategory.TshirtsPolos, condition: ProductCondition.TresBonEtat, size: 'XL', city: 'Casablanca', images: ['https://picsum.photos/seed/p16a/600/800'], seller: mockUsers[0], status: ProductStatus.Sold },
...
export const mockOrders: Order[] = [
    { id: 'o1', product: mockProducts.find(p => p.id === 'p16')!, buyer: mockUsers.find(u => u.id === 'admin')!, seller: mockUsers[0], status: OrderStatus.Shipped, totalAmount: 329, shippingFee: 35, buyerProtectionFee: 14, createdAt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString(), updatedAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString() },
    { id: 'o2', product: mockProducts.find(p => p.id === 'p6')!, buyer: mockUsers.find(u => u.id === 'admin')!, seller: mockUsers[2], status: OrderStatus.Completed, totalAmount: 140, shippingFee: 35, buyerProtectionFee: 5, createdAt: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000).toISOString(), updatedAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString() },
    { id: 'o3', product: mockProducts.find(p => p.id === 'p3')!, buyer: mockUsers[1], seller: mockUsers.find(u => u.id === 'admin')!, status: OrderStatus.Paid, totalAmount: 124, shippingFee: 35, buyerProtectionFee: 9, createdAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString(), updatedAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString() },
]
