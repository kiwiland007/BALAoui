...
interface EditItemPageProps {
  product: Product;
  onUpdateItem: (product: Product) => void;
  onCancel: () => void;
}
...
const EditItemPage: React.FC<EditItemPageProps> = ({ product, onUpdateItem, onCancel }) => {
  const [title, setTitle] = useState(product.title);
  const [description, setDescription] = useState(product.description);
  const [price, setPrice] = useState(product.price.toString());
  const [originalPrice, setOriginalPrice] = useState(product.originalPrice?.toString() || '');
  const [category, setCategory] = useState<ProductCategory>(product.category);
  const [condition, setCondition] = useState<ProductCondition>(product.condition);
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl">
      <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-sm">
        <h1 className="text-3xl font-bold text-text-main dark:text-secondary mb-6 text-center">Modifier votre article</h1>
