...
const UserTable: React.FC<UserTableProps> = ({ users, onSearchChange, onShowToggleProConfirm }) => {
    return (
        <>
            <CardHeader>
                    ...
                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-3">Statut Pro</th>
                                ...
                                <tr key={user.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td className="px-6 py-4">
                                        {user.isPro ? (
                                            ...
                                            <span className="text-text-light">Standard</span>
                                        )}
                                    </td>
                                    <td className="px-6 py-4 font-semibold text-text-main dark:text-secondary">{user.balance.toFixed(2)} MAD</td>
                                    <td className="px-6 py-4">{user.memberSince}</td>
                                    <td className="px-6 py-4 space-x-1">
                                         <Button variant="outline" className="h-8 px-3 text-xs" onClick={() => onShowToggleProConfirm(user)}>
                                            {user.isPro ? <><i className="fa-solid fa-arrow-down mr-2"></i>Révoquer</> : <><i className="fa-solid fa-crown mr-2"></i>Promouvoir</>}
                                        </Button>
                                    </td>
                                </tr>
                            ))}
